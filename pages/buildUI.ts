 ...
 export class BuildUIPage {
  ...
  async navigate() {
   ...
   async buildid() {
     // await expect(this.page.getByText('Build ', { exact: true })).toBeVisible({ timeout: 30000 });
 ...
 console.log('New tab URL:', this.newTab.url());
       ...
       , num: string
        ...
        //  await newTab.locator('button:has-text("Submit")').click({timeout: 10000});
       ...
       // await newTab.locator('button:has-text("Submit")').click({timeout: 10000});
         ...
         //range
         /* async setSliderValue(targetValue: number, min = 0, max = 100) */
      ...
      const dialog = this.page.locator('mat-dialog-container[role="dialog"]');
       ...
       if (responseRowCount > 0) { 
         ...
         responseSectionText = await responseBody.innerText();
       }
